...
from hallmark_mlx.inference.warm_start_planner import (
    build_query,
    build_tool_calls,
    next_action_for_tool_calls,
)
from hallmark_mlx.types import InputType, SuspectedIssueCode
...
def _venue_from_citation(raw_text: str, year: int | None) -> str | None:
    cleaned = _DOI_RE.sub(" ", raw_text)
    segments = _segments_from_citation(cleaned)
    if len(segments) < 3:
        return None
    venue_index = len(segments) - 1
    venue_segment = segments[venue_index]
    if venue_segment.lower() == "doi" and venue_index > 0:
        venue_segment = segments[venue_index - 1]
    if year:
...
class WarmStartPolicyModel:
    ...
    def propose_trace(self, verification_input: VerificationInput) -> VerificationTrace:
        ...
        return VerificationTrace(
            ...
            next_action=next_action_for_tool_calls(tool_calls),
